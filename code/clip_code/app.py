"""
CLIP Retrieval Test Site — Datix AI
Single-model test page for CLIP ViT-B/32 (OpenAI), using the retrieval
approach: photos are matched by finding the closest stored embedding in the
gallery index exported from the Kaggle benchmark notebook.
...
NOTE: The exported zip intentionally does NOT bundle CLIP's own weights
(multi-hundred MB) — this script pulls them fresh from Hugging Face on
first run via from_pretrained(), same as the CLIP test site built earlier.
That download is cached locally afterward.
"""
...
# app.py lives in: D:\zen_cart\code\clip_code\
...
# The actual Hugging Face repo — NOT the same as the export folder name above.
# This is hardcoded here rather than read from model_info.json, since that
# file's 'hf_repo_hint' field only stores the local folder name, not the
# real loadable repo id.
HF_REPO = "openai/clip-vit-base-patch32"
MODEL_DISPLAY_NAME = "CLIP ViT-B/32 (OpenAI)"
...
# ---------------------------------------------------------------------------
# Load the precomputed gallery index (fast — no model needed for this part)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Lazy-loaded live model — only pulled into memory on first prediction
# ---------------------------------------------------------------------------
_clip_model = None
...
def predict(pil_image, top_k=TOP_K):
    ...
    sims = _gallery_prototypes @ query_embed  # cosine similarity (both L2-normalized)
